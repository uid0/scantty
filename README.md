...
- Review preventive maintenance from Maintenance: the due board separates the rolling seven-day window from the rest of the next 30 days and can ask OMS to generate all due work orders in one confirmed action; the rollup shows scheduled work, active maintenance, and costs. An asset's detail sheet also carries its combined work-order and historical maintenance log, where staff can add a backdated record or edit its notes.
