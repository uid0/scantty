...
func (s *PurchaseOrderAddLineScreen) identifyHeader(essential bool) jdeHeader {
	...
	// `renderJDEField` with the SHARED column, never a band builder over these
	// two rows alone: a band recomputes jdeLabelWidth over only the fields it is
	// handed — "Supplier", eight cells against poAddLabelWidth()'s twelve. The
	// label is right-aligned into that width, so these two pinned rows and their
	// dotted leaders drew four cells to the LEFT of the `Scan / type` row
	// directly beneath them: the one shared label column the columnar convention
	// rests on, broken on the phase an operator drives with a scanner. It also cost the VALUES four
