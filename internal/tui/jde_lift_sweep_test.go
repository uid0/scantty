...
// ---------------------------------------------------------------------------
// A ceiling is a claim about another bar, never a second copy of it
// ---------------------------------------------------------------------------

// TestJDEForm_NoBarCeilingRestatesABarLiteral is the other half, and it is the
// half with teeth on the defect that motivated it.
//
// barCeiling's loading and summary arms used to spell barFor's literals word for
// word. Nothing was wrong on screen — the two literals were identical — and that
// is exactly the trouble: an item added to one of those bars alone would have
// left the header allowance measured against a bar shorter than the one on the
// pane, and no state, no height and no key could have reported it. The rendered
// sweep above only notices a divergence big enough to FOLD the bar, so on its
// own it would pass a restated literal that had drifted by one short item.
//
// So the rule is asked of the SOURCE, where it is actually decided: no arm of a
// barCeiling may return a bar it built itself. A ceiling is a claim ABOUT
// another bar, so it has exactly one honest shape — a call to whatever builds
// that bar — and a composite literal there is a second copy of a legend, free to
// drift from the first in silence.
//
// Derived over every barCeiling in the package rather than over the one that was
// reported: location_reconcile's arms already delegate, and a rule applied only
// where it was reported is the failure mode this project keeps recording.
//
// It is the DISPATCHERS it judges, and the per-phase qtyBarCeiling /
// serialBarCeiling / countBarCeiling / rowBarCeiling / reviewBarCeiling beside
// them are literals on purpose rather than by omission. Each is the UNION of
// every optional item at its longest wording, and the builder that draws the bar
// gates those items on predicates the ceiling deliberately ignores — a stock
// level, a refusal, a row count — so there is no argument that makes the drawn
// builder produce the union and nothing to derive it FROM. What the arms above
// restated was a bar barFor already builds whole, which is the difference.
func TestJDEForm_NoBarCeilingRestatesABarLiteral(t *testing.T) {
	fset, files := jdeParsePackage(t)
	swept, returns := 0, 0
	for _, f := range files {
		for _, decl := range f.Decls {
			fn, ok := decl.(*ast.FuncDecl)
			if !ok || fn.Recv == nil || fn.Name.Name != "barCeiling" || fn.Body == nil {
				continue
			}
			swept++
			ast.Inspect(fn.Body, func(n ast.Node) bool {
				...
				for _, res := range ret.Results {
					returns++
					if _, lit := res.(*ast.CompositeLit); lit {
						t.Errorf("%s: this arm of barCeiling builds its own bar instead of "+
							"calling the one that draws it, so the two are free to drift "+
							"and nothing on any pane would say so",
							fset.Position(res.Pos()))
					}
				}
				return true
			})
	...
	if swept == 0 {
		t.Fatal("no barCeiling was found, so this judged nothing")
	}
	if returns == 0 {
		t.Fatal("no barCeiling arm was read, so this judged nothing")
	}
}
