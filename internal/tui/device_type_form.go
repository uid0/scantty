...
func (s *DeviceTypeFormScreen) formLines() *jdeLines {
	...
	for i, f := range fields {
		...
		if s.fields[i] == dtName && len(fixed) > 0 {
			...
			l.AddFittedField(jdeNoRow, fixed[0], labelWidth, s.bodyWidth())
		}
...
func (s *DeviceTypeFormScreen) formBarItems(paging bool) []actionBarItem {
...
func (s *DeviceTypeListScreen) paneCells() int { return proseBarCells(s.terminalWidth) }
...
func (s *DeviceTypeListScreen) bar(moves bool) proseBar {
...
func (s *DeviceTypeListScreen) proseBar() proseBar {
...
func (s *DeviceTypeListScreen) loadBar() proseBar {
...
func (s *DeviceTypeListScreen) View() string {
	if s.loading {
		return proseLoadingFrame("Loading device types…", s.paneCells(), s.proseBar())
	}
	if s.loadErr != "" {
