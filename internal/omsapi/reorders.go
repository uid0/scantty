...
type PurchaseOrderItem struct {
	...
	// The (item, supplier) relationship this line was placed against, which is
	// where the SUPPLIER's part number lives. It is nil on an asset line and on
	// a freeform one — neither names an ItemSupplier — so a caller must tell
	// "this line has no supplier relationship" from "the relationship carries a
	// blank SKU"; POLineItemSupplier's doc comment carries that distinction.
	ItemSupplierDetails *POLineItemSupplier `json:"item_supplier_details,omitempty"`
...
// POLineItemSupplier is the (item, supplier) relationship a catalogue purchase
// order line was placed against — OMS's item_supplier_details, a nested
// ItemSupplierSerializer on PurchaseOrderItemSerializer.
//
// ONLY SupplierSKU is decoded, because it is the only thing on that block this
// program has a reader for; the relationship's pricing and packaging reach
// ScanTTY through the paths that already fetch an ItemSupplier in full.
//
// THE SUPPLIER SKU IS NOT THE ITEM'S OWN SKU, and the two are a live confusion
// on a PO line. item_details.sku is the makerspace's INTERNAL part number,
// generated by OMS when nobody supplies one; this is what the VENDOR calls the
// part, and it is the string a buyer quotes when placing the order.
// OpenMakerSuite settles which is which on its own order-pad export, which
// builds every row from po_item.item_supplier.supplier_sku and reports a
// blank one in missing_sku rather than dropping the row
// (backend/reorder_queue/views.py, export_order).
//
// IT IS ALSO NOT item_details.supplier_sku, which InventoryItemSerializer
// carries as a flat legacy accessor for the item's PRIMARY supplier. A purchase
// order placed with anyone but the primary supplier would take the wrong
// vendor's part number from that key, which is the whole reason this block is
// decoded rather than the flat one read.
//
// WIRE TYPE, derived from the builder and not from the model (AGENTS.md):
// supplier_sku is ItemSupplier.supplier_sku, a plain
// models.CharField rendered by a stock ModelSerializer, so it is a JSON
// string on every payload and never a number. It has no blank=True, so OMS
// refuses to save a blank one — but a row predating that constraint can still
// serialise as "", which is why every reader treats "" as an absence rather
// than asserting the field is populated.
type POLineItemSupplier struct {
