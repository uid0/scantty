...
// PostMultipart posts a multipart/form-data body — zero or more text fields
// plus at most one file part — to path and decodes the JSON response into out.
// It exists for the endpoints that only accept multipart (the PO
// upload-attachment action, whose backend view sets parser_classes to
// MultiPartParser/FormParser), where the JSON-only do() path can't help. Like
// do() it retries once after a token refresh on 401. The body is buffered so
// the redirect-preserving policy can replay it when an http:// base
// 301-upgrades to https:// (see preserveMethodOnRedirect).
func (c *Client) PostMultipart(
	ctx context.Context, path string, fields map[string]string,
	fileField, fileName string, file io.Reader, out any,
) error {
	...
	if file != nil {
		...
		if err != nil {
			return fmt.Errorf("oms: multipart file part: %w", err)
		}
		if _, err := io.Copy(fw, file); err != nil {
			return fmt.Errorf("oms: multipart copy: %w", err)
		}
	}
	if err := mw.Close(); err != nil {
		return fmt.Errorf("oms: multipart close: %w", err)
	}
	return c.doMultipart(ctx, path, buf.Bytes(), mw.FormDataContentType(), out, true)
}

// doMultipart is the multipart sibling of do(): it POSTs an already-marshalled
// body with a caller-supplied Content-Type (which carries the multipart
// boundary), refreshes+retries once on 401, and decodes the JSON response.
// GetBody + ContentLength are set so the redirect policy can re-send the body.
func (c *Client) doMultipart(ctx context.Context, path string, body []byte, contentType string, out any, retryAuth bool) error {
	...
	if resp.StatusCode == http.StatusUnauthorized && retryAuth && c.refresh != "" {
		if rerr := c.Refresh(ctx); rerr == nil {
			return c.doMultipart(ctx, path, body, contentType, out, false)
		}
	}
	if resp.StatusCode >= 400 {
