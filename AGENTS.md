...
### "WORK ORDER" NAMES TWO DIFFERENT THINGS AND TWO DIFFERENT DJANGO APPS

The same collision the ForgeKey note at the top of this file is about, one
workspace over, and it is already load-bearing in the code: `wo_detail.go` and
`vendor_work_order_detail.go` are two screens with the same noun in their names
driving two unrelated state machines.

1. **An IN-HOUSE work order** — `inventory.WorkOrder`, the shop doing the job
   itself. `omsapi/workorders.go`, `internal/tui/wo_detail.go`, the Maintenance
   workspace's landing list (`kind: "work_orders"`). Its siblings are
   `MaintenanceItem` (the recurring PM task, `omsapi/maintenance.go`) and the
   PM board.
2. **A VENDOR work order** — `maintenance_orders.ThirdPartyWorkOrder`, a
   PURCHASE: somebody comes on site, an invoice arrives, money leaves.
   `omsapi/maintenance_orders.go` is the client and carries the contract note;
   `internal/tui/vendor_work_order_detail.go` is the seven-step stepper and
   `vendor_work_orders.go` the list (`kind: "vendor_work_orders"`, the
   Maintenance workspace's "Vendor work orders" nav surface).

`asset_problems.go` promotes a reported problem into EITHER, which is where the
two meet and where reading one comment as being about the other costs the most.

What is worth knowing before touching the vendor half:

- **The seven-step state machine is the SERVER's and is never re-derived.**
  `backend/maintenance_orders/transitions.py` owns every gate. What this side
  READS is the `workflow` block `ThirdPartyWorkOrderSerializer.get_workflow`
  computes (has_nte, has_required_quotes, has_photo_evidence,
  has_invoice_and_fsr, variance_status, keyfob_outstanding) — the same block the
  web stepper renders its gates from. Two of those facts cannot be computed here
  at all (the 24-hour emergency window needs the server's clock, the attachment
  gates need the rows) and one would be WRONG if computed: "three quotes" is not
  the rule, it is one of three ways to satisfy it.
- **Every transition is one-way.** There is no backward transition, no unsign,
  no unwaive, no unvoid and no reopen anywhere in `transitions.py`, and
  `authorize_emergency`'s window only expires — `revoked_at` is reachable from
  the Django admin and from no endpoint. That is why the terminal confirms every
  write rather than only the money-shaped ones, and why `Ctrl-X` rather than
  `Enter` commits at the confirm (the `po_edit.go` rule: the key that OPENS a
  prompt must not be the key that commits it).
- **`kind` on an attachment is a GATE, not a filing label.** `photo` is what
  sign-off requires and `invoice` + `fsr` together are what closure requires, so
  a file put under the wrong kind leaves the gate shut with nothing erroring.
  Whatever drives the workflow must therefore drive the ATTACHMENT upload too,
  or it reaches step 4 and stops.
- **The refusal body is NOT the standard envelope**, for the fourth time in this
  file: `views._err` writes `{"detail": <prose>}` by hand — as a STRING for one
  message and a LIST for several (`detail[0] if len(detail) == 1 else detail`) —
  so `omsapi.parseError` hands the whole raw body over.
  `omsapi.AsMaintenanceOrderRefusal` recovers both shapes and is deliberately
  narrow, so a gateway page and a field-validation envelope keep the shape they
  arrived in.
- **Every pk in the `maintenance_orders` app is an explicit `models.UUIDField`,
  and `location` is the one exception on the payload.** `inventory.Location`
  declares no pk, so it is `settings.DEFAULT_AUTO_FIELD` and arrives as a JSON
  NUMBER while everything beside it is a UUID string — the wire-type rule above,
  in one payload. That is why nothing here is an `any`.
- **`record-keyfob-return` and `override-variance` are not in the web stepper's
  eleven actions and the terminal drives them anyway.** Each closes a dead end
  the eleven would otherwise leave: a keyfob recorded at `vendor-arrived` blocks
  closure until it is returned, and a blocked variance blocks it until it is
  overridden. Offering the arming action without the clearing one is building
  the wall this work was filed to remove.
