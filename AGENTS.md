...
### A purchase-order line has TWO forms, and the CONTENT picks which

`internal/tui/po_detail.go` (`poBuildOneLineGrid`) is the authority and
`internal/tui/po_one_line_test.go` is the sweep. What is worth knowing before
touching the line grid, or any layout whose behaviour changes past 120 columns:

- **The BLOCK form is what 80 columns gets and it did not change.** A line is a
  grid row, then its readings, then the free-text rows that deserve one of their
  own (`poLineBlock`). Give the pane enough cells and the whole block goes on ONE
  row, carrying three facts no width of the block form shows: the SUPPLIER's part
  number, the part's whole 36-cell UUID, and `estimated_cost` — the price the
  supplier is charging for the line, which is the LINE's own total and not the
  per-unit or per-case figure (see "Purchase-order line money has two
  denominators" above, which owns that distinction).
- **The threshold is the CONTENT's, and nothing compares the pane against a
  number.** The rows are ASSEMBLED, measured in display cells in one forward
  pass, and the widest one is the threshold — so a field added to the row later
  moves it by exactly what it costs, where a constant would go on claiming a fit
  the row had stopped having. Measured: the shared three-line fixture collapses
  from 251 terminal columns, and a single line carrying every optional row from
  356. Those are OUTPUTS of those fixtures' values, which is why the sweeps
  report them and assert the boundary's SHAPE instead of a number.
- **It answers for the ORDER and not for the row being drawn** — the maximum
  over every line, the construction `voidCaveatsFit` already uses, and here for
  a second reason: the columns are budgeted from the whole set, so a per-line
  answer would draw rows that do not line up under their own headings.
- **It REFUSES rather than mutilates, and there is no shortened form.** A part
  UUID is never abbreviated (the captain's standing instruction) and a supplier
  SKU is a `CharField(100)`, so an order carrying a long one simply keeps the
  block form. That is what makes the wide form a pure ADDITION: below the
  threshold — 80 included — the pane is what it was before it existed. A KIT
  line keeps the whole order on the block form at every width, because its
  credit block is prose naming several OTHER items and there is no row for it to
  ride.
- **NO EXISTING SWEEP COULD REACH THIS STATE, and that is the part to carry
  forward.** Every derived width axis in `internal/tui` stops at 120 —
  `jdeDrawableWidths` caps there because "a wider pane only folds less", which
  was true of every layout in the program until this one — so `jdePaneWidths`,
  `poViewWidths` and the pane-fit sweeps all measure the block form and are
  silent about the other one. A layout whose behaviour changes past 120 columns
  needs a width axis of its own, and each sweep in `po_one_line_test.go` counts
  the panes it found on BOTH sides of the boundary and fails if it never reached
  one — because a sweep that only ever saw the block form passes with the whole
  wide form deleted.
- **THE TWO NEW COLUMNS TELL AN ABSENCE FROM A SILENCE** (`poOneLineNone` vs
  `poOneLineUnknown`). A freeform line has no vendor part number and no part id
  and never will — that is the order as somebody wrote it — while a catalogue
  line whose reply arrived WITHOUT its `item_supplier_details` block has both
  and we were not told them, which is a lookup to go and do. The state is
  reachable rather than theoretical: that block is nested, so an OMS predating
  it answers every line with the relationship id and no block, and a column
  reading only the block would draw "no SKU" down a whole order of catalogue
  lines. `li.ItemSupplier` / `li.Asset` — the bare pks — are what separate them.

- **THREE part-number-shaped strings ride a PO line and only ONE is the
  vendor's.** `item_details.sku` is the shop's INTERNAL number, generated by OMS
  when nobody supplies one; `item_details.supplier_sku` is a flat legacy
  accessor for the item's PRIMARY supplier, which is the wrong vendor entirely
  on an order placed with anyone else; the right one is
  `item_supplier_details.supplier_sku`, which OMS builds every row of its own
  order-pad export from (`backend/reorder_queue/views.py`, `export_order`, whose
  blank ones go to `missing_sku` rather than being dropped). The block form's
  `PART` reading is the INTERNAL one and its comment claimed it was what a buyer
  quotes at the supplier — a documented claim the code did not honour, and the
  reason this distinction is written down rather than left to be rediscovered.
