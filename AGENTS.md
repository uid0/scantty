...
### An asset METER is a number the server will let you get wrong

`internal/omsapi/asset_meters.go` carries the contract note,
`internal/tui/asset_meter_sanity.go` the guard, and
`internal/omsapi/testdata/README.md` the recorded bodies and the method; all
three are the authority. What is worth knowing before touching anything
meter-shaped:

- **`record-reading` and `adjust` are DIFFERENT OPERATIONS and the record keeps
  them apart.** A reading says what the machine reads NOW; an adjustment
  CORRECTS the history and the server requires a reason it stores on the row.
  They land in the same append-only ledger as `manual` and `manual_adjust`, so
  the distinction survives for whoever reads it months later. Collapsing them
  into one call with a flag is cheaper and destroys that at the point it is made.
- **THE SERVER REFUSES NEITHER A BACKWARDS READING NOR AN ORDER-OF-MAGNITUDE
  ONE.** Measured, not inferred: absolute 120 against a meter reading 1250.5,
  absolute 12505000, and absolute -5 are each a **201**. `apply_reading`
  computes `delta = value_after - current_value` and stores what it is handed —
  no monotonicity rule, no magnitude bound, nothing to relay.
- **AND ON A `runtime_hours` METER THE DAMAGE IS ONE-WAY**, which is what decides
  that the terminal asks first. `apply_reading` adds a positive advance to
  `Asset.hours_used` through an `F()` update and deliberately does NOT decrement
  on the way down; `Asset.hours_used` is what the existing maintenance forecast
  reads. Measured: 1250.5 → 120 → 12505000 → -5 left `hours_used` at 12506130,
  and NO endpoint in this client brings it back — `adjust` fixes the METER and
  never those hours. So the confirm NAMES and never refuses: nothing alters a
  value, nothing declines to send one, and one more key (`Ctrl-X`, not the
  `Enter` that opened the frame) sends exactly what was typed.
- **THE BACKWARDS TEST IS OFF ON `adjust` AND THE MAGNITUDE TEST IS NOT.** Going
  backwards is what a correction is FOR — a recount that came in lower is the
  textbook case — so a confirm there would fire on the correct use of the
  instrument and teach the operator to dismiss it. A slipped digit is a slipped
...
- **A FAILED OR IN-FLIGHT REFRESH MAKES THE DISPLAYED VALUE UNCHECKED, NOT
  UNUSABLE.** The screen keeps the last meter list so the operator does not lose
  what they were looking at, but every record or adjustment then goes through
...
- **A NUMBER IS SHOWN WHOLE OR DROPPED AND MARKED.** The value column is a FACT
  ...
  own with the header marked. `meterFigureLine` owns the one place the two rules
  compete — at the 16 cells a 45-column terminal leaves, the digits are never
  cut and the UNIT is what gives, marked. Never `jdeWrapTokens` for a figure:
  that helper CLIPS a token too wide for its line.
- **EVERY VALUE CARRIES ITS UNIT**, on the grid, in both forms' hints, on the
  confirm and on the status row. A runtime-hour reading and a cycle count are
  not interchangeable. The unit is free text server-side and may be blank; the
  server's own meter-TYPE label stands in, never one invented here.
- **Every id is a UUID and therefore a STRING**, and `current_value` / `delta` /
  `value_after` are decimal STRINGS at four places (`numeric(14,4)`). The
  arithmetic is `big.Rat`, never `float64` — the rule `po_case_entry.go` states
  for money, for the same reason.
- The refusals are DRF `{"detail": …}` written by hand in the view body, so
  `AsDetailRefusal` recovers the sentence. The DOCUMENT viewset next door is a
  plain `ModelViewSet`, so ITS refusals are OMS's standard
  `{"error": {code, message, details}}` envelope — two shapes, one screen apart.
- `internal/tui/asset_documents.go` is the library beside it. **Superseding keeps
  both rows and deleting destroys one**, which is why exactly one of them is
  confirmed: a document another version supersedes can be deleted out from under
  that link (`on_delete=SET_NULL`) and the chain loses a rung with nothing
  recording it. Superseded versions are SHOWN and MARKED rather than filtered
  out — the terminal's job here is to stop somebody following a stale manual.
  A blank title/category/description on `supersede` is OMITTED and never sent
  empty: the server `setdefault`s an absent key from the prior document, so an
  empty one would not mean "leave it alone", it would mean "untitled".
- `internal/omsapi/asset_meters_lab_test.go` (build tag `omslab`) drives all
  eight endpoints against a REAL backend, the acceptance of a backwards and an
  order-of-magnitude reading included — so if OMS ever starts refusing them, the
  confirm's premise fails loudly rather than quietly becoming decoration.
